...
class Sample(object):
    ...
    def run_debug(self, colored = True, palette: tuple = ("#43c928", "#c98928", "#c93328"), tests_outputs: bool = False) -> None:
        ...
        print(f'Result: {passed}/{total} ({round((passed * 100) / total, 2)}%) [PASSED {passed} | FAILED {failed} | CRASH {crashed}]' if (not colored) else f'Result: {bold_color}{passed}{eol_color}/{bold_color}{total}{eol_color} ({bold_color}{round((passed * 100) / total, 2)}{eol_color}%) [{transformed_palette[0]}PASSED{eol_color} {bold_color}{passed}{eol_color} | {transformed_palette[1]}FAILED{eol_color} {bold_color}{failed}{eol_color} | {transformed_palette[2]}CRASH{eol_color} {bold_color}{crashed}{eol_color}]')
